require_relative '../repositories/repository_entry'
require_relative './solution'

class SubmissionsOthers
  ...
  def submissions
    @submissions ||= RepositoryEntry.submissions(
      challenge_id: challenge_id,
  ...
  def bellow_player_score
    score = RepositoryEntry.best_player_score(challenge_id, player.id)
    if score
      RepositoryEntry.bellow_score(challenge_id, score)
    else
  ...
  def worst_score
    @worst_score ||= RepositoryEntry.worst_score(challenge_id)
  end
  ...
  def count_remaining
    @count_remaining ||= RepositoryEntry.count_remaining_solutions(challenge_id, visible_score)
  end

  def count_displayed
    @count_displayed ||= RepositoryEntry.count_displayed_solutions(challenge_id, visible_score)
  end
