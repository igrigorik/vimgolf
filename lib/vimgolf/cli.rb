module VimGolf
	...
	no_tasks do
		...
		begin
			...
			res = Net::HTTP::Proxy(uri.host, (uri.port ? uri.port : 80), proxy_user, proxy_pass).start(url.host, url.port) {|http| http.request(req)}
		...
		begin
			...
			proxy_user, proxy_pass = uri.userinfo.split(/:/) if uri.userinfo
			Net::HTTP::Proxy(uri.host, (uri.port ? uri.port : 80), proxy_user, proxy_pass).start(url.host, url.port) do |http| 
